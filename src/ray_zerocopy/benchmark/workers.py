...
@ray.remote
def normal_task_worker(model_bytes_ref, batches, batch_size, use_jit: bool = False):
    """Normal task worker that loads model from serialized bytes."""
    ...
    # model = torch.load(model_bytes, map_location="cpu", weights_only=False)
...
class NormalActorRayData:
    """Normal actor for Ray Data that loads model from serialized bytes (full copy)."""
