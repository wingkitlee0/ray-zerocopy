...
def ray_put_model(model: torch.nn.Module | torch.jit.ScriptModule, use_jit: bool = False) -> ray.ObjectRef:
    if not use_jit:
...
def ray_get_model(model_ref: ray.ObjectRef | bytes, use_jit: bool = False, **kwargs) -> torch.nn.Module | torch.jit.ScriptModule:
