...
def run_ray_core_normal(model, workers, batches, batch_size, use_jit: bool = False):
    ...
    with monitor_memory_context(interval=1.0) as memory_stats:
        ...
        futures = [
            normal_task_worker.remote(model_ref, batches, batch_size, use_jit=use_jit)
...
def run_ray_data_normal(model, workers, batches, batch_size, use_jit: bool = False):
    ...
    with monitor_memory_context(interval=1.0) as memory_stats:
        ...
        results = ds.map_batches(
            NormalActorRayData,
            fn_constructor_kwargs={"model_ref": model_ref, "use_jit": use_jit},
            batch_size=1,
