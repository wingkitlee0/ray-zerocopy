...
2. Task-based - Using ModelWrapper.for_tasks() or JITTaskWrapper
3. Actor-based - Using ModelWrapper.from_model(..., mode="actor") or JITActorWrapper
...
def main():
    ...
    try:
        if args.mode == "ray_core":
            ...
            approaches = {
                ...
                "task_based": lambda: run_ray_core_task_based(
                    model, args.workers, args.batches, args.batch_size, create_task_wrapper
                ),
                "actor_based": lambda: run_ray_core_actor_based(
                    model, args.workers, args.batches, args.batch_size, create_actor_wrapper
                ),
            }
            ...
            approaches = {
                "normal": lambda: run_ray_data_normal(
                    model, args.workers, args.batches, args.batch_size
                ),
                "task_based": lambda: run_ray_data_task_based(
                    model, args.workers, args.batches, args.batch_size, create_task_wrapper
                ),
                "actor_based": lambda: run_ray_data_actor_based(
                    model, args.workers, args.batches, args.batch_size, create_actor_wrapper
                ),
            }
        ...
        save_results_json(
            results, config, args.mode, args.output, use_jit=args.use_jit
        )
